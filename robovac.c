...
static BOOL IsSupportedModSignature(const UBYTE *header)
{
    return ((header[1080] == 'M' && header[1081] == '.' && header[1082] == 'K' && header[1083] == '.') ||
            ...
            (header[1080] == 'F' && header[1081] == 'L' && header[1082] == 'T' && header[1083] == '4') ||
            (header[1080] == 'N' && header[1081] == '.' && header[1082] == 'T' && header[1083] == '.'));
}
...
static void SetPaulaSampleLoop(WORD channel, struct ModSample *sample)
{
    ULONG base;
    ULONG addr;
    ULONG loopOffset;
    ...
    loopOffset = 0;
    loopLength = 2;
    addr = (ULONG)introMod.silentSample;

    if (sample && sample->data && sample->length >= 2 &&
        sample->repeatLength > 2 && sample->repeatOffset < sample->length) {
        loopOffset = sample->repeatOffset;
        loopLength = sample->repeatLength;
        if (loopOffset + loopLength > sample->length) {
            loopLength = sample->length - loopOffset;
        }
        if (loopLength >= 2) {
            addr = (ULONG)(sample->data + loopOffset);
        } else {
            loopLength = 2;
            addr = (ULONG)introMod.silentSample;
        }
    ...
    PokeCustomWord(base + 0x00, (UWORD)(addr >> 16));
    PokeCustomWord(base + 0x02, (UWORD)(addr & 0xFFFF));
    PokeCustomWord(base + 0x04, (UWORD)(loopLength >> 1));
}

static void ApplyPendingIntroModLoops(void)
{
    WORD channel;

    for (channel = 0; channel < MOD_CHANNELS; channel++) {
        if (introMod.channels[channel].pendingLoop) {
            SetPaulaSampleLoop(channel, introMod.channels[channel].sample);
            introMod.channels[channel].pendingLoop = FALSE;
        }
    }
}
...
static void FreeIntroMusic(void)
{
    ...
    if (introMod.silentSample) {
        FreeMem(introMod.silentSample, 2);
    }
...
static BOOL LoadIntroMusic(void)
{
    ...
    introMod.silentSample = AllocMem(2, MEMF_CHIP | MEMF_CLEAR);
    if (!introMod.silentSample) {
        Close(file);
        FreeIntroMusic();
        return FALSE;
...
static void StartIntroMusic(void)
{
    ...
    for (i = 0; i < MOD_CHANNELS; i++) {
        ...
        introMod.channels[i].pendingLoop = FALSE;
